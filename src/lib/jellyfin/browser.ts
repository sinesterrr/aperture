...
function web0sVersion(browser: BrowserInfo): number | undefined {
...
export const detectBrowser = (
  ...
  const browser: BrowserInfo = {
    versionMajor: matched.versionMajor,
  };
  ...
  browser.edgeChromium =
    (browser as any).edg || (browser as any).edga || (browser as any).edgios;
  ...
  browser.tizen =
    normalizedUA.includes("tizen") || (window as any).tizen != null;
  ...
  browser.edgeUwp =
    ((browser as any).edge || browser.edgeChromium) &&
    (normalizedUA.includes("msapphost") || normalizedUA.includes("webview"));
  ...
  if (browser.iOS) {
    ...
    browser.iOSVersion =
      ver && ver.length >= 2 ? ver[0] + ver[1] / 10 : undefined;
  }
