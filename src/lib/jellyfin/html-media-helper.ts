...
export function getSavedVolume() {
    return parseInt(appSettings.get('volume') || '1', 10);
}
...
export function enableHlsJsPlayerForCodecs(mediaSource: any, mediaType: string) {
    ...
    if (!browser.iOS && browser.safari && mediaSource.MediaStreams.some((x: any) => x.Codec === 'vp9')) {
        return true;
    }
...
export function enableHlsJsPlayer(runTimeTicks: any, mediaType: string) {
    if ((window as any).MediaSource == null) {
        return false;
    }
...
export function handleHlsJsMediaError(instance: any, reject?: (reason?: any) => void) {
    const hlsPlayer = instance._hlsPlayer as Hls;
...
export function onErrorInternal(instance: any, type: string) {
    // Needed for video
...
function setCurrentTimeIfNeeded(element: HTMLMediaElement, seconds: number) {
...
export function seekOnPlaybackStart(instance: any, element: HTMLMediaElement, ticks: number, onMediaReady?: () => void) {
...
export function applySrc(elem: HTMLMediaElement, src: string, options: any) {
    if ((window as any).Windows && options.mediaSource?.IsLocal) {
        return Windows.Storage.StorageFile.getFileFromPathAsync(options.url).then(function (file: any) {
...
export function resetSrc(elem: HTMLMediaElement) {
    elem.src = '';
    elem.innerHTML = '';
    elem.removeAttribute('src');
}

function onSuccessfulPlay(elem: HTMLMediaElement, onErrorFn: (e: any) => void) {
    elem.addEventListener('error', onErrorFn);
}

export function playWithPromise(elem: HTMLMediaElement, onErrorFn: (e: any) => void) {
    try {
        return elem.play()
            .catch((e) => {
...
export function bindEventsToHlsPlayer(instance: any, hls: Hls, elem: HTMLMediaElement, onErrorFn: (e: any) => void, resolve: any, reject: any) {
...
export function onEndedInternal(instance: any, elem: HTMLMediaElement, onErrorFn: (e: any) => void) {
...
export function getBufferedRanges(instance: any, elem: HTMLMediaElement) {
    const ranges = [];
