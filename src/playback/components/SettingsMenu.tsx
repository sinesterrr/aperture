    ...
    useEffect(() => {
        async function fetchTracks() {
            if (currentItem?.Id && currentMediaSource?.Id) {
                const audio = await getAudioTracks(currentItem.Id, currentMediaSource.Id);
                const subs = await getSubtitleTracks(currentItem.Id, currentMediaSource.Id);
            ...
            <DropdownMenuContent className="w-64 bg-black/90 text-white border-gray-800 z-[100] max-h-[80vh] overflow-y-auto">
                ...
                <DropdownMenuSub>
                    <DropdownMenuSubTrigger>
                        ...
                        <span className="text-xs text-gray-400 ml-2">
                             {playbackState.preferredQuality === 'auto' ? 'Auto' : 
                              playbackState.preferredQuality === 'direct' ? 'Direct' : 
                              qualityOptions.find(q => String(q.bitrate) === playbackState.preferredQuality)?.name || playbackState.preferredQuality}
                        </span>
                    </DropdownMenuSubTrigger>
                ...
                <DropdownMenuSub>
                    <DropdownMenuSubTrigger>
                        ...
                        <span className="text-xs text-gray-400 ml-2">{playbackState.playbackRate}x</span>
                    </DropdownMenuSubTrigger>
                    <DropdownMenuSubContent className="bg-black/90 text-white border-gray-800">
                        <DropdownMenuRadioGroup value={String(playbackState.playbackRate)} onValueChange={handleSpeedChange}>
                ...
                <DropdownMenuSub>
                    <DropdownMenuSubTrigger>
                        ...
                        <span className="text-xs text-gray-400 ml-2">{playbackState.aspectRatio}</span>
                    </DropdownMenuSubTrigger>
                    <DropdownMenuSubContent className="bg-black/90 text-white border-gray-800">
                        <DropdownMenuRadioGroup value={playbackState.aspectRatio} onValueChange={handleAspectRatioChange}>
                            <DropdownMenuRadioItem value="contain">Auto (Contain)</DropdownMenuRadioItem>
                            <DropdownMenuRadioItem value="cover">Cover (Zoom)</DropdownMenuRadioItem>
                            <DropdownMenuRadioItem value="fill">Fill (Stretch)</DropdownMenuRadioItem>
                        </DropdownMenuRadioGroup>
                    </DropdownMenuSubContent>
                </DropdownMenuSub>
